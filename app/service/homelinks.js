...
class HomeLinksService extends Service {
  ...
  async getList(limit) {
    let result = null;
    if(limit){
      const sql = " select * from home_links LIMIT "+ limit;
      result = await this.app.mysql.query(sql);
    }else{
      result = await this.app.mysql.select('home_links', {
        orders: [[ 'id', 'desc' ]], // 排序方式
      });
    }
  ...
  // 根据用户id查询数据
  async findByID(id) {
    const result = await this.app.mysql.get('home_links', { id });
    return { data: result };
  }
  ...
  async addModel(data) {
    ...
    // 新增数据
    const result = await app.mysql.insert('home_links', data);
    return {
  ...
  async updateModel(data) {
    ...
    const { ctx, app } = this;
    data.createtime = ctx.helper.currentDateTime();
    const result = await app.mysql.update('home_links', data);
    return {
      error_code: result.affectedRows > 0 ? 0 : 1,
  ...
  async destroyModel(id) {
    const result = await this.app.mysql.delete('home_links', { id });
    return {
      error_code: result.affectedRows > 0 ? 0 : 1,
