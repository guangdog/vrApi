...
//  }
  ...
  async LoginByEmail(email, password) {
    const result = await this.app.mysql.get('common_users', { email, password });
    return {
      error_code: result ? 0 : 1,
      data: result,
    };
  ...
  //   const result = await this.app.mysql.get('tb_users', { phone, password });
  //   return {
  //     error_code: result ? 0 : 1,
  //     data: result,
  //   };
  ...
  //   // 判断是否保存过验证码
  //   const isHaveCode = await this.app.mysql.get('yzm', { id: phone });
  ...
  //     let res = null;
  //     if (isAdd) {
  //       res = await this.app.mysql.insert('yzm', data);
  //     } else {
  //       res = await this.app.mysql.update('yzm', data);
  //     }
  //     // 判断结果
  ...
  //   const result = await this.app.mysql.query(sql, [ data.password, data.phone ]);
  //   return {
  //     error_code: result.affectedRows > 0 ? 0 : 1,
  ...
  //   const result = await this.app.mysql.get('tb_users', { phone });
  //   return result;
  // }
  // 判断用户邮箱是否存在
  async isHaveUserEmail(email) {
    const result = await this.app.mysql.get('common_users', { email });
    return result;
  }
  // 判断用户名是否存在
  async isHaveUserName(username) {
    const result = await this.app.mysql.get('common_users', { username });
    return result;
  }
}
